...
async function analizaLatitude(latitude, longitude) {
  ...
  dados.localizacao = await Banco.session(`select m.latitude, m.longitude from vendi.coodmunicipio m  where m.latitude like '${dados.latitude}%' and longitude like '${dados.longitude}%' order by id_coodmunicipio desc`)
  dados.localizacao = dados.localizacao.rows
  dados.distancia = Funcoes.distanciaLatLong(latitude, longitude, dados.localizacao.latitude, dados.localizacao.longitude)
  return dados
}
...
async function vendedor(token) {
    ...
    if(user.status== false){
      ...
      if(vendedor.result.rows[0]){
        ...
        vendedor.result = vendedor.result.rows
        return vendedor      
      }
