...
async function criatelefone(){
  const banco= await Banco.session("CREATE TABLE Vendi.telefone (id_telefone SERIAL CONSTRAINT pk_id_telefone PRIMARY KEY,id_pessoa integer REFERENCES vendi.pessoa (id_pessoa),telefone varchar(20) not null, whatsapp boolean NOT NULL);")
...
async function criaendereco(){
  const banco= await Banco.session("CREATE TABLE Vendi.endereco (id_endereco SERIAL CONSTRAINT pk_id_endereco PRIMARY KEY,id_pessoa integer REFERENCES vendi.pessoa (id_pessoa), rua varchar(50) NOT NULL, bairro varchar(50) NOT NULL, cidade varchar(50) NOT NULL, cep varchar(8) NOT NULL, numero integer NOT NULL);")
...
async function criavendedor(){
  const banco= await Banco.session("CREATE TABLE Vendi.vendedor (id_vendedor SERIAL CONSTRAINT pk_id_vendedor PRIMARY KEY,id_pessoa integer REFERENCES vendi.pessoa (id_pessoa), classificacao integer NOT NULL);")
...
async function criacategoria(){
  const banco= await Banco.session("CREATE TABLE Vendi.categoria (id_categoria SERIAL CONSTRAINT pk_id_categoria PRIMARY KEY, descricao varchar(50) NOT NULL);INSERT INTO vendi.categoria(descricao)VALUES ('Geral'); INSERT INTO vendi.categoria(descricao)VALUES ('Automoveis'); INSERT INTO vendi.categoria(descricao)VALUES ('Servicos');")
...
async function criafoto(){
  const banco= await Banco.session("CREATE TABLE Vendi.foto (id_foto SERIAL CONSTRAINT pk_id_foto PRIMARY KEY,id_anuncio integer REFERENCES vendi.anuncio(id_anuncio),linkfoto varchar(200)not null);")
...
async function criaentrega(){
  const banco= await Banco.session("CREATE TABLE Vendi.entrega (id_entrega SERIAL CONSTRAINT pk_id_entrega PRIMARY KEY,id_vendedor integer REFERENCES vendi.vendedor (id_vendedor),id_endereco integer REFERENCES vendi.endereco (id_endereco),codigoRastreio varchar(50) not null, tipoEntrega integer not null, dataConfirmacaoEntrega date not null)")
...
async function criaformadepagamento(){
  const banco= await Banco.session("CREATE TABLE Vendi.formadepagamento (id_formadepagamento SERIAL CONSTRAINT pk_id_formadepagamento PRIMARY KEY, descricao varchar(50) NOT NULL);")
...
async function userTeste(password) {
  const banco    = await Banco.session("INSERT INTO Vendi.user(email, senha, nome)VALUES ('teste@teste.com',$1,'Desenvolvedor');",[password])
  return true
}
